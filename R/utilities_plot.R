plotFun <- function(myPath,summaryTab,idxAnimal){
  ...
  for(idxRamping in unique(currentTab$ramping)){
    ...
    for(angleSelect in uniqueAngles){
      plot.200khz <- subset(TS.kde.200khz,angleMatch == angleSelect & ramping == idxRamping)
      ...
      plot.70khz  <- subset(TS.kde.70khz,angleMatch == angleSelect & ramping == idxRamping)
    ...
    for(angleSelect in uniqueAngles){
      plot.all <- subset(TS.stats.all,angleMatch == angleSelect & ramping == idxRamping)
