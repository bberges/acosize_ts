...
plt.savefig('loss_profile'+species+'_angle.png', dpi=300)
plt.close()
...
pd.DataFrame(data=outFrame,columns=['angle','length','length_pred']).to_csv('G:/acosize_ts/results/pred_'+species+'_angle_dodgy.csv', index=False)
#
plt.scatter(oupt[0,:], target[:,0])
plt.ylabel('targets (cm)')
plt.xlabel('predictions (cm)')
plt.title('SAI')
plt.savefig('pred_'+species+'_angle.png', dpi=300)
plt.close()
#
# minAngle = 240
# maxAngle = 280
