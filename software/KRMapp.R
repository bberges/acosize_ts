...
list.of.packages <- c("shiny","grid","tidyverse","ggplot2", "magick","sf")
...
server <- function(input, output) {
  ...
  ppshp = observeEvent(input$pshp,{
    output$krmshp <- renderPlot({
      ...
      if("x_sb" %in% names(shpdf)){
        ...
        KRMr::shplot(x_fb = shpdf$x_fb, w_fb = shpdf$w_fb,
  ...
  observeEvent(input$rKRM,{
    ...
    TS =KRMr::krm.sim(frequency = seq(input$freq_min, input$freq_max,length.out=input$freq_n) * 1000,
                      c.w = input$cw,
                      rho.w = input$rhow,
